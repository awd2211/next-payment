...
func main() {
	...
	// 初始化OCR客户端
	ocrServiceURL := getConfig("OCR_SERVICE_URL", "http://localhost:40030")
	ocrClient := client.NewOCRClient(ocrServiceURL)
	logger.Info("OCR服务客户端初始化完成", zap.String("url", ocrServiceURL))

	// 3. 初始化 Repository
	kycRepo := repository.NewKYCRepository(application.DB)

	// 4. 初始化 Service
	kycService := service.NewKYCService(application.DB, kycRepo, notificationClient, ocrClient)
